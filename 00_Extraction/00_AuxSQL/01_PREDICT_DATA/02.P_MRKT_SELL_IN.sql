...
/*#########################################################################*/
IF OBJECT_ID('[STAGING_2].[dbo].XXX_P_Mrkt_Sell_IN_neg', 'U') IS NOT NULL
 DROP TABLE [STAGING_2].[dbo].XXX_P_Mrkt_Sell_IN_neg;
...
into [STAGING_2].[dbo].XXX_P_Mrkt_Sell_IN_neg
...
/*#########################################################################*/
IF OBJECT_ID('[STAGING_2].[dbo].XXX_P_Mrket_Sell_IN_ajust', 'U') IS NOT NULL
 DROP TABLE [STAGING_2].[dbo].XXX_P_Mrket_Sell_IN_ajust;
...
with Mrkt_Sell_IN_std as (			
 ...
 --n.CAL_DATE,n.SI_WSE,
			...
			from [ITE_PRD].[ITE].[V_FACT_Sales_Target_WSE_Daily] t
				join	ITE.T_DAY	d
				 on 	(t.[DIA] = d.[DIA])
			 join [STAGING_2].[dbo].XXX_P_Mrkt_Sell_IN_neg n 
			...
			where  t.MIDCATEGORY in (N'BLOND', N'RYO') and t.[Mrkt_WSE] > 0 and t.DIA > 20141200
...
into [STAGING_2].[dbo].XXX_P_Mrket_Sell_IN_ajust
from  [ITE_PRD].[ITE].[V_FACT_Sales_Target_WSE_Daily] t
				join	ITE.T_DAY	d
				 on 	(t.[DIA] = d.[DIA])	      
