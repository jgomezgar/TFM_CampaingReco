...
/*  RESULT: Datos entrenamiento ventas SellIn 
   [STAGING_2].[dbo].XXX_P_ITG_Sell_IN --> Sell In Positivos, ventas base
   [STAGING_2].[dbo].XXX_P_ITG_Sell_IN_neg --> Sell In Negativos, determina Familias&Clitentes a arreglar
...
IF OBJECT_ID('[STAGING_2].[dbo].XXX_P_ITG_Sell_IN', 'U') IS NOT NULL
 DROP TABLE [STAGING_2].[dbo].XXX_P_ITG_Sell_IN;
...
into [STAGING_2].[dbo].XXX_P_ITG_Sell_IN
...
/*#########################################################################*/
IF OBJECT_ID('[STAGING_2].[dbo].XXX_P_ITG_Sell_IN_neg', 'U') IS NOT NULL
 DROP TABLE [STAGING_2].[dbo].XXX_P_ITG_Sell_IN_neg;
...
into [STAGING_2].[dbo].XXX_P_ITG_Sell_IN_neg
from	ITE.FACT_SMLD_Smoke_ITG	a11
	join	ITE.T_DAY	a12
	  on 	(a11.[SALESDATE] = a12.[CAL_DAY])
	join	ITE.T_BRANDPACKS	a15
...
/*################################ 33 min #####################################*/
IF OBJECT_ID('[STAGING_2].[dbo].XXX_P_ITG_Sell_IN_Periods', 'U') IS NOT NULL
 DROP TABLE [STAGING_2].[dbo].XXX_P_ITG_Sell_IN_Periods;
...
with ITG_Sell_IN_std as (			
		...
		from (
			...
			from [STAGING_2].[dbo].XXX_P_ITG_Sell_IN t
			 join [STAGING_2].[dbo].XXX_P_ITG_Sell_IN_neg n 
...
--into [STAGING_2].[dbo].XXX_P_ITG_Sell_IN_Periods
from  [STAGING_2].[dbo].XXX_P_ITG_Sell_IN t	      
