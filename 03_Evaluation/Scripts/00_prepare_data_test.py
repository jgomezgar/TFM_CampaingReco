...
INPUT = '../../01_Preparation/Data/sampled_test.h5'
INPUT_FEATS = '../../02_Training/Scripts/features.npy'
OUTPUT = '../Data/prepared_test.h5'

# Read merged data for sampling
X = pd.read_hdf(INPUT, key='sampled_test')
...
features = list(np.load(INPUT_FEATS))
X = X[features]

X.to_hdf(OUTPUT, key='prepared_test', index=False, mode='w')
